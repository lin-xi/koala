(function (window) {
	...
	Mvc.prototype.config = function (opts) {
		...
		runController('AppController', document.body);
		if (opts.plugins) {
			//DIRECTIVE.plugins
		}
	};
	...
	Mvc.prototype._route = function (path) {
		...
		if (rt && rt.template) {
			_.ajax.get(_config.base + rt.template, {
				t: +new Date().getTime()
			}, function (response) {
				if (_config.view) {
	...
	function loadModule(names) {
		if (names.length > 0) {
			for (var i = 0, len = names.length; i < len; i++) {
				var path = _config.base + names[i] + '.js';
				_.loadJs(path);
			}
		}
	}
		...
		_.each(_parseTree, function (key, treeNode) {
			...
			if (node) {
				if (treeNode.bindTimes) {
					...
					var arrStr = _.trim(items[1]),
						itemStr = _.trim(items[0]);
